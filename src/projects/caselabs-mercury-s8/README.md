...
| Drive set            | `FI-HDD-001`; eight operator-reported WD VelociRaptor 450 GB drives acquired for the mount around 2014                                                        | Registered set / four `WD4500BLHX-01V7BV0` drives directly identified / four pending inspection          | Test every drive by direct SATA: identity, SMART report, short and extended self-tests, and a complete read pass; no array admission on USB-dock evidence alone |
| Drive carriers       | `FI-HDD-ICEPACK-001`; operator-identified WD IcePack 2.5-to-3.5-inch mounting frames and heat sinks                                                           | Registered set / exact count, part number, revision, adapter hardware, and thermal-pad condition pending | Photograph and reconcile each frame, then map one qualified drive and carrier assembly to each pedestal bay                                                     |
...
registered separately until their exact labels, revisions, and completeness are
photographed. A separate 1 TB Seagate SSHD that produced kernel I/O errors is
excluded from this subsystem.
...
- [ ] Photograph and reconcile all eight drive labels and every retained
      IcePack frame without publishing serial numbers.
- [ ] Assign non-public test handles `VR-01` through `VR-08` after label and
      condition capture.
