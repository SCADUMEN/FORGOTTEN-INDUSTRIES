...
- Accessioned into `SCADUMEN/FORGOTTEN-INDUSTRIES` on 2026-08-27 from the operator working copy. Entries LA-0001 through LA-0017 carried over unchanged in substance.
- **This tracked copy is redacted.** This repository is public. Unit-identifying strings — drive PSIDs, drive and module serial numbers, WWN/EUI64 identifiers, and vendor asset tags — are replaced with `[withheld — private register]`, following the convention already stated in `src/data/inventory.yml` ("serial numbers remain in the private register"). Model numbers, part numbers, S-Spec codes, date codes, PCB revisions, and batch markings are retained: those establish catalog identity and are already published across the inventory.
- The unredacted master remains in the operator's local working copy, outside the public tree. Restore a value here only by deliberate decision, not by default.
- LA-#### numbers are a raw intake sequence. They are **not** the site accession numbers (`ACCESSION 0XX`) or the durable object IDs (`FI-HW-…`). For the mapping between the two, and for the entries the canonical record already resolves, see `src/docs/intake/2026-08-27-larchive-accession-crosswalk.md`.
...
- Lot: FA 1651PGS; unit string `[withheld — private register]`
...
- PSID: `[withheld — private register]`
- WWN: `[withheld — private register]`
- Rated DC+5V, 1.4A
...
- EUI64: `[withheld — private register]`
- SN: `[withheld — private register]` (0/O ambiguous on label; transcribed as read in the private copy)
- PSID: `[withheld — private register]`
- Date code: 2026.06
- Rated DC 3.3V, 1.85A
- Product of Vietnam
...
- Board barcode: `[withheld — private register]` (label on PCB, likely an assembly/serial tag)
...
- Additional label data: 9905403-400.A00LF; serial, asset tag, and lot strings `[withheld — private register]`
...
- Made in Taiwan, ver 5.21, lot string `[withheld — private register]`
...
- Additional label data: 9905403-877.A00LF; lot, serial, and asset tag strings `[withheld — private register]`; date code 1528
...
- "AMD Athlon(TM) II"
- Part: ADX2200CK22GM
- Batch: CADHC AD 1015GPMW; unit string `[withheld — private register]`
- Diffused in Germany, Made in Malaysia
...
**Cooling:** An EK monoblock (LA-0017) is stated to belong to this board — found separately in the same bin as LA-0016, not physically paired with the board in any photo taken so far. **This pairing is contradicted by the canonical record; see LA-0017 and the crosswalk.**
...
**Block manufacturer — closed by canonical record (2026-08-27):** The site inventory identifies this assembly as an EK-FC670 GTX full-cover block on the EVGA GTX 660 Ti (`FI-HW-GPU-660TI-001`), which closes the "block manufacturer not confirmed" flag above in favor of EK, not Koolance. The Koolance reading applies to the fittings only, as this entry already suspected.
...
**Fitment — contradicted by canonical record (2026-08-27):** The site inventory carries this block as `FI-HW-BLOCK-Z87-001`, an **EK-FB ASUS M6I** monoblock for the ASUS ROG **Maximus VI** Impact (`FI-HW-MB-Z87-001`, Intel Z87) — an identifier the archive explicitly corrected from `FI-HW-BLOCK-Z97-001` on manufacturer-reference evidence, and which the FI-PROJ-009 dossier summary states as settled. The "belongs to the Maximus VII Impact" reading in this entry and in LA-0015 is the reading that correction overturned. Treated as **closed against the canonical record**, not reopened: the Maximus VI Impact is a separately held board, so the two Impacts are distinct objects and the block follows the VI. Reopen only if a direct part-number photograph contradicts the site record.

**Condition:** Loose, unmounted. Not tested with the board.

**Next action:** A clean, direct photo showing a printed model number would convert the canonical EK-FB ASUS M6I identification from reference-based to label-verified.
